...
public interface AuditLogRepository
    ...
    findByActionContainingIgnoreCaseAndStatusContainingIgnoreCaseAndActorEmailContainingIgnoreCaseAndTargetEmailContainingIgnoreCase(
            String action, String status, String actorEmail, String targetEmail, Pageable pageable
    );
    ...
    findByActionContainingIgnoreCaseAndStatusContainingIgnoreCaseAndActorEmailContainingIgnoreCaseAndTargetEmailContainingIgnoreCaseAndCreatedAtBetween(
            String action, String status, String actorEmail, String targetEmail,
    ...
    // ================= USER HISTORY =================
    List<AuditLog> findByTargetEmailOrderByCreatedAtDesc(String targetEmail);
    ...
    @Query("""
        ...
        AND a.action = 'LOGIN' AND a.status = 'FAILED'
        AND a.createdAt >= :fromTime
        """)
    long countRecentFailedLogins(String email, LocalDateTime fromTime);

    // ================= USER RECENT ACTIVITY =================
    List<AuditLog> findTop5ByActorEmailOrderByCreatedAtDesc(String actorEmail);

    // ================= USER ACTIVITY PAGINATION =================
    Page<AuditLog> findByActorEmailOrderByCreatedAtDesc(String actorEmail, Pageable pageable);

    // ================= DAY 65 — Filtered personal activity =================
    @Query("""
        SELECT a FROM AuditLog a
    ...
    Page<AuditLog> findByActorEmailWithFilters(
            String actorEmail, String action, String status, Pageable pageable
    );
    ...
    @Query("""
        ...
        """)
    List<Object[]> countActionTrend(String action, LocalDateTime startDate);

    // ✅ Day 72 — Total actions by a specific actor email
    long countByActorEmail(String actorEmail);

    // ✅ Day 72 — Total logins (SUCCESS) by actor email
    @Query("""
        SELECT COUNT(a) FROM AuditLog a
        WHERE LOWER(a.actorEmail) = LOWER(:email)
        AND a.action = 'LOGIN' AND a.status = 'SUCCESS'
        """)
    long countSuccessfulLoginsByEmail(String email);

    // ✅ Day 72 — Most recent successful login
    @Query("""
        SELECT a FROM AuditLog a
        WHERE LOWER(a.actorEmail) = LOWER(:email)
        AND a.action = 'LOGIN' AND a.status = 'SUCCESS'
        ORDER BY a.createdAt DESC
        """)
    List<AuditLog> findRecentSuccessfulLogins(String email, Pageable pageable);
}